...
static inline void __flush_icache(void)
{
	...
	__asm__ __volatile__ ("movi	%0, 0x31\n\t"
	                      "mtcr	%0, cr17"
	                      : "=r" (__b)); 
}
 ...
 * Instruction Cache handling functions
 */
static inline void __clear_icache(void)
{
	register long __b;
	
	__asm__ __volatile__ ("movi	%0, 0x21\n\t"
	                      "mtcr	%0, cr17"
	                      : "=r" (__b)); 
}
...
#define flush_icache_all()    __flush_icache()
#define flush_dcache_all()    __flush_dcache()
#define clear_icache_all()    __clear_icache()
#define clear_dcache_all()    __clear_dcache()
